...
- [x] **Phase 8h — Agent Control Room (trace) + critic agent**: every AD-console pipeline run is recorded as ordered, inspectable steps (`agent_runs`/`agent_steps` tables; `app/trace.py` RunRecorder that swallows its own failures and re-raises pipeline ones) and rendered at `GET /agents` (AD-auth, "Agents" nav) — `/debug/gcp` stays the receipt, `/agents` is the story. Instrumented flows: incident text/voice/manual, edit, sandbox groups, publish/revert (approve = `kind=human`), weather adopt. Critic agent (`app/agents/critic.py`): one schema-enforced Gemini call over all sandbox option stats → `{recommended_strategy, confidence, reasons}` with a deterministic cheapest-feasible fallback; rendered as a recommendation chip + banner that states the contract ("may only recommend — can never reorder scenes or change feasibility"); every path logged to `/debug/gcp` as kind `critic`. 213 tests green.

- [x] **Phase 8i — Crew Response Agent + Exceptions queue**: crew personal links gain inbound replies — `POST /c/{token}/reply` (text via one schema-enforced Gemini call: `ack|running_late|cannot_make|constraint|question` + `eta_minutes` + `leave_by`) and `POST /c/{token}/reply/voice` (same call, WAV in memory via a `data-voice-action` override on the shared voice widget) — plus a manual quick-reply form with the exact parity contract. Raw audio is NEVER persisted (evidence logs carry lengths only); personal narrative never renders on AD pages beyond structured fields. Dashboard Exceptions queue: unhandled replies critical-first (DP / 1st AD / Sound Mixer / Key Grip / Script Supervisor via `CRITICAL_ROLES`), mark-handled, and "Generate recovery options" for critical `cannot_make`/`late>30`, which files a `CAST_DELAY` incident through the standard pipeline (never mutates plans). Replies require a published plan and reject expired/rotated tokens like acks. 223 tests green.

- [x] **Phase 8j — Sentinel agent + governance dial**: pure signal evaluators (`app/sentinel.py`) — `daylight_burn_down` (remaining EXT/DAY minutes vs sun left), `meal_penalty_clock` (live plan past meal deadline with no lunch), `ack_staleness` (T+10 advisory, T+20 critical-role escalation to a `CAST_DELAY` incident), `weather_imminent` (fixture/cache hazard window starting ≤60 min over remaining EXT work), `slippage` (unhandled crew `running_late` ≥30 min). `POST /sentinel/tick` (AD-auth, poller-friendly, no background threads) is idempotent per (signal, day) via the `sentinel_state` table and materializes incidents ONLY through the existing pipeline (sandbox for blocking, review plan otherwise). Governance dial: `GOVERNANCE_MODE=advise|auto_low|off` (default `advise` never publishes; `off` disables; `auto_low` publishes the cheapest feasible option only below BOTH `AUTO_PUBLISH_MAX_USD`/`AUTO_PUBLISH_MAX_MINUTES_MOVED` ceilings — missing cost data is fail-closed) with the skipped human gate trace-logged. Dashboard gains a read-only Sentinel card evaluating signals on render. 239 tests green.
